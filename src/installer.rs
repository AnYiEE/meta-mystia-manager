...
impl<'a> Installer<'a> {
    ...
    fn execute_install_cleanup(game_root: &std::path::Path, ui: &dyn Ui) -> Result<(usize, usize)> {
        ...
        if failed > 0 {
            ...
            ui.warn("  部分文件删除失败，将继续安装")?;
        } else {
            ui.message(&format!("旧版本删除完成（清理 {} 项）", success))?;
        }
    ...
    pub fn install(&self, cleanup_before_deploy: bool) -> Result<()> {
        // 1. 获取版本信息
        self.ui.display_step(1, "获取版本信息");
        let version_info = self.downloader.get_version_info()?;
        self.ui.display_version_info(&version_info);

        // 2. 获取分享码
        self.ui.display_step(2, "获取下载链接");
        ...
        let install_resourceex = if cleanup_before_deploy {
            ...
            if resourceex_exists {
                ...
                self.ui.ask_install_resourceex()?
            }
        } else {
            self.ui.ask_install_resourceex()?
        };
        ...
        self.ui.message("所有文件下载完成")?;

        // 5. 在安装前清理旧版本
        if cleanup_before_deploy {
            self.ui.message("")?;
            self.ui.message("正在清理旧版本...")?;
            Self::execute_install_cleanup(&self.game_root, self.ui)?;
        }

        // 6. 安装文件
        self.ui.display_step(4, "安装文件");
        ...
        self.ui.message("安装完成！")?;
        self.ui.message("现在可以启动游戏，Mod 将自动加载。")?;
